...
| VCC       | 电源      | 5V            | **强烈推荐接 5V**（因 W5500 峰值电流可达近 200mA，ESP32 自带的 3.3V LDO 供电常常不足导致无响应或掉线） |
| GND       | 地        | GND           | 与 ESP32 共地                |

注意：

- **供电至常重要**：请尽量避免将 W5500 接到 ESP32-C3 开发板的 `3.3V` 引脚。由于电流常常不足，会导致网关无法初始化（`hardwareStatus: 0`）。推荐直接连接 `5V`。W5500 的 SPI 控制引脚均兼容 5V 容忍，直接连接 ESP32 是安全的。
- **MAC 地址动态分配**：为了防止在复杂的局域网环境中由于固定出厂 MAC 导致 ARP 冲突或被路由器交换机拉黑，网关现在会**自动读取 ESP32 的出厂 WiFi MAC 地址**作为以太网通信的真实物理地址，保证全网唯一性。
- **CS 引脚冲突**：由于 ESP32-C3 底层硬件控制器的限制，在代码中初始化 SPI 时已经做了专门处理（不在 `SPI.begin` 中传入 CS 引脚，改用软件 `digitalWrite` 触发），避免了 `Ethernet` 库无法拉低 CS引脚而导致的通信全部失败。
