...
static const char PAGE_CSS[] PROGMEM =
    ...
    "#ccc;border-radius:4px;margin-bottom:10px;width:300px} "
...
static const char REWARD_MODAL_HTML_PART1[] PROGMEM =
...
static void handleHttpRoot() {
  ...
  html += "<label>云端提取密码:</label><input type='password' id='syncPassword' placeholder='输入密码一键同步' style='vertical-align:middle'>"
          "<button type='button' id='btnDoSync' onclick='uiDoCloudSync()' style='margin-left:10px;padding:6px 12px;background:#4f46e5;color:white;border:none;border-radius:4px;cursor:pointer;vertical-align:middle;font-size:13px;margin-bottom:10px'>一键填充</button>"
          "<span id='syncStatus' style='font-size:12px;color:#666;margin-left:10px;vertical-align:middle;display:none'></span><br>";
  ...
  g_httpServer.sendContent(html);
  html = "";
  g_httpServer.sendContent_P(REWARD_MODAL_HTML_PART1);
  g_httpServer.sendContent(g_otaApiBase);
  g_httpServer.sendContent_P(REWARD_MODAL_HTML_PART2);
  ...
  html += "async function uiDoCloudSync(){"
          "  var pwd=document.getElementById('syncPassword').value.trim();"
          "  var statusDiv=document.getElementById('syncStatus');"
          "  var btn=document.getElementById('btnDoSync');"
          "  if(!pwd){"
          "    statusDiv.style.display='inline-block';"
          "    statusDiv.style.color='#ef4444';"
          "    statusDiv.innerText='⚠️ 请输入提取密码';"
          "    return;"
          "  }"
          "  statusDiv.style.display='inline-block';"
          "  statusDiv.style.color='#666';"
          "  statusDiv.innerText='⏳ 正在拉取配置...';"
          "  btn.disabled=true;"
          "  try{"
          "    let res=await fetch('https://update.anyport.one/anyport/mqtt-config',{"
          "      method:'POST',"
          "      headers:{'Content-Type':'application/json'},"
          "      body:JSON.stringify({code:pwd})"
          "    });"
          "    if(!res.ok){"
          "      let errData=await res.json().catch(()=>({error:'网络错误'}));"
          "      throw new Error(errData.error||'请求失败 ('+res.status+')');"
          "    }"
          "    let data=await res.json();"
          "    let urlStr=data.brokerUrl||'';"
          "    if(urlStr){"
          "      try{"
          "        let normalizedUrl=urlStr.replace(/^(mqtt|mqtts|ws|wss):\\/\\//i,'http://');"
          "        if(!normalizedUrl.startsWith('http://')&&!normalizedUrl.startsWith('https://')){"
          "          normalizedUrl='http://'+normalizedUrl;"
          "        }"
          "        let url=new URL(normalizedUrl);"
          "        document.getElementsByName('mqttHost')[0].value=url.hostname;"
          "        let port=url.port;"
          "        if(!port){"
          "          port=(urlStr.startsWith('wss:')||urlStr.startsWith('mqtts:'))?'8883':'1883';"
          "        }"
          "        document.getElementsByName('mqttPort')[0].value=port;"
          "        document.getElementsByName('mqttPath')[0].value=url.pathname;"
          "      }catch(e){"
          "        document.getElementsByName('mqttHost')[0].value=urlStr;"
          "      }"
          "    }"
          "    if(data.username!==undefined)document.getElementsByName('mqttUser')[0].value=data.username;"
          "    if(data.password!==undefined)document.getElementsByName('mqttPwd')[0].value=data.password;"
          "    if(data.topicPrefix!==undefined)document.getElementsByName('mqttPrefix')[0].value=data.topicPrefix;"
          "    if(data.siteId!==undefined)document.getElementsByName('mqttSite')[0].value=data.siteId;"
          "    if(data.gatewayId!==undefined)document.getElementsByName('mqttGateway')[0].value=data.gatewayId;"
          "    statusDiv.style.color='#10b981';"
          "    statusDiv.innerText='✨ 提取成功，配置已自动填充！';"
          "    setTimeout(function(){"
          ...
          "  }catch(err){"
          "    statusDiv.style.color='#ef4444';"
          "    statusDiv.innerText='⚠️ 同步失败: '+err.message;"
          "  }finally{"
          "    btn.disabled=false;"
          "  }"
          "}";
  html += "document.getElementById('syncPassword').addEventListener('keydown',function(e){"
          "  if(e.key==='Enter')uiDoCloudSync();"
          "});";
